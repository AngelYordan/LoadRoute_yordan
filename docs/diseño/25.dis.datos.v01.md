...
## 1. Entidades del Dominio y Persistencia MySQL
El sistema actual usa MySQL como persistencia de los datos cargados (`aeropuertos`, `vuelos`, `envios`). Para la ejecucion directa de la simulacion, el backend convierte esos registros a modelos de dominio en memoria y asi mantiene velocidad en miles de iteraciones algoritmicas.
...
Los resultados de jobs asincronos son temporales y no se guardan en MySQL; solo viven en memoria hasta que se descargan, se eliminan o expira su TTL.
